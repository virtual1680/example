<template>
  <div class="dv-scroll-board" :ref="ref">
    ...
    <div v-if="mergedConfig" class="rows" :style="`height: ${height - (header.length ? mergedConfig.headerHeight : 0)}px;`">
      <div class="row-item" v-for="(row, ri) in rows" :key="`${row.toString()}${row.scroll}`" :style="`
          height: ${heights[ri]}px;
          line-height: ${heights[ri]}px;
          background: ${mergedConfig[sIndex===row.rowIndex?'selectedBGC':row.rowIndex % 2 === 0 ? 'evenRowBGC' : 'oddRowBGC']};
        `">
